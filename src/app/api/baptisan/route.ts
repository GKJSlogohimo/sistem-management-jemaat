import {
  baptisanListQuerySchema,
  createBaptisanSchema,
} from "@/features/baptisan/schemas/baptisan.schema";
import { createBaptisan, getBaptisanList } from "@/features/baptisan/server/baptisan.service";
...
export async function GET(request: Request) {
  try {
    ...
    const params = baptisanListQuerySchema.parse(Object.fromEntries(url.searchParams.entries()));

    const result = await getBaptisanList(createActor(authActor), params);
...
export async function POST(request: Request) {
  try {
    ...
    const input = createBaptisanSchema.parse(await request.json());

    const result = await createBaptisan(createActor(authActor), input);

    return apiSuccess(result, {
      status: 201,
      message: "Data Baptisan berhasil ditambahkan.",
    });
  } catch (error) {
    return handleApiError(error);
  }
